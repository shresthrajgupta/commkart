...
const ProductEditPage = () => {
    ...
    return (
        <>
            ...
            <FormContainer>
                <Link className="btn btn-light my-3" to="/admin/productlist" style={{ backgroundColor: "#F7B733", border: "none", color: "white" }}> Go Back </Link>

                <div style={{ backgroundColor: "white", padding: "20px", borderRadius: "5px" }}>


                    <h1 style={{ color: "#3c3d40" }}> Edit Product </h1>
                    ...
                    {getProductDetailsLoading ? <Loader /> : (getProductDetailsErr ? (<Message variant='danger'> {getProductDetailsErr?.data?.message || getProductDetailsErr?.error} </Message>) : (
                        <Form onSubmit={updateProductSubmitHandler}>
                            ...
                            <Form.Group controlId="price" className="my-2">
                                ...
                                <Form.Control type="number" placeholder="Enter price" value={price} onChange={(e) => setPrice(e.target.value)} className="custom-input" />
                            </Form.Group>

                            <Form.Group controlId="image" className="my-2">
                                <Form.Label> Image </Form.Label>
                                <Form.Control type="text" placeholder="Enter image url" value={image} onChange={(e) => setImage(e.target.value)} className="custom-input" />
                            ...
                            <Form.Group controlId="brand" className="my-2">
                                ...
                                <Form.Control type="text" placeholder="Enter brand" value={brand} onChange={(e) => setBrand(e.target.value)} className="custom-input" />
                            </Form.Group>

                            <Form.Group controlId="countInStock" className="my-2">
                                <Form.Label> Count In Stock </Form.Label>
                                <Form.Control type="number" placeholder="Enter count" value={countInStock} onChange={(e) => setCountInStock(e.target.value)} className="custom-input" />
                            </Form.Group>

                            <Form.Group controlId="category" className="my-2">
                                <Form.Label> Category </Form.Label>
                                <Form.Control type="text" placeholder="Enter category" value={category} onChange={(e) => setCategory(e.target.value)} className="custom-input" />
                            </Form.Group>

                            <Form.Group controlId="description" className="my-2">
                                <Form.Label> Description </Form.Label>
                                <Form.Control type="text" placeholder="Enter description" value={description} onChange={(e) => setDescription(e.target.value)} className="custom-input" />
                            </Form.Group>

                            <Button type="submit" variant="primary" className="my-2" style={{ backgroundColor: "#F7B733", border: "none", color: "white" }}> Update </Button>
                        </Form>
                    ))}
                </div>
            </FormContainer>
